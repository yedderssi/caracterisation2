...
class MeasurementLogger:
    ...
    def __init__(self, metadata, root_dir="./Logs"):
        ...
        print(f"\U0001F4C1 Logger initialisé : {self.dir}")
    ...
    def log_iteration(self, scalars, waveforms):
        ...
        # 2) Sauvegarde des waveforms au format CSV (au lieu du .npz binaire).
        #    -> 1 colonne = 1 signal, 1 ligne = 1 échantillon, lisible dans Excel.
        ...
        for cle, arr in waveforms.items():
            ...
            if arr.ndim <= 1:
                ...
                # Matrice 2D (ex. Mat_Broyden) : enregistrée dans son propre CSV.
                mat_path = os.path.join(self.dir, f'{cle}_iter_{iter_num:03d}.csv')
        ...
        if signaux_1d:
            ...
            wf_path = os.path.join(self.dir, f'waveforms_iter_{iter_num:03d}.csv')
    ...
    def finalize(self, status, best_iteration=None):
        ...
        print(f"✅ Session enregistrée : {self.dir}")
...
# --- Sonde de courant Keysight N2783B sur voie D ---
USE_PROBE_D    = True    # False pour revenir au comportement 3 voies
PROBE_V_PER_A  = 0.1     # sensibilité N2783B : 0,1 V/A
PROBE_N_TOURS  = 1       # nb de passages du conducteur dans la pince (3-5 conseillé)
PROBE_SIGN     = +1      # -1 si la flèche de la pince est dans l'autre sens
MESURE_OFFSETS_AU_DEMARRAGE = True  # True : acquisition excitation OFF au lancement pour
...
PK_MAX_GBF = 5.0           # PLAFOND DUR d'amplitude envoyée au GBF (V).
                           # À AJUSTER selon: tension max GBF, gain ampli, et limite courant primaire.
...
class Worker(QThread):
    ...
    finished = pyqtSignal()
    update_plot = pyqtSignal(dict)   # BUG2 : signal thread-safe pour rafraîchir l'UI
    ...
    def run(self):
        ...
        from scipy.signal import get_window,detrend, correlate, correlation_lags,  hilbert, savgol_filter, sawtooth, butter, resample
        ...
        def THDN(signal, sample_rate):
            ...
            # CORRECTION : signal nul → THD = 0, évite NaN et le crash
            if total_rms < 1e-10:
               return 0.0
        ...
        def couper_excitation(gbf):
            ...
            for tentative in range(2):
                try:
                    ...
                    print(f"[SECURITE] echec OUTP OFF (essai {tentative+1}): {e}")
                    try:
                        gbf.close()
                    except Exception:
                        pass
                    gbf = None
            print("[SECURITE] !!! coupure logicielle impossible — COUPER L'AMPLI A LA MAIN !!!")
            return None
        ...
        def GBF(entree, gbf, allumer=True):
            ...
            for _ in range(20):
                ...
                if errorCode == 0:
                    break
                msg = parts[1].rstrip('\n') if len(parts) > 1 else ''
                print(f'INSTRUMENT ERROR - code {errorCode}: {msg}')
                gbf.write('*CLS')
        ...
        def Mesure(Selection_Channel, Channel_ranges, chandle):
            ...
            for idx_channel, Numero_Channel in enumerate(Selection_Channel):  # Boucle séléction channel
                ...
                while True:
                    ...
                    duration = Nbre_periode / Frequence  # Durée de la fenêtre d'acquisition
                    P = num_samples // Nbre_periode       # points cibles par période

                    # Calcul du timebase PUIS dérivation de maxSamples pour que la fenêtre
                    # contienne EXACTEMENT Nbre_periode périodes (élimine la fuite spectrale).
                    # maxSamples est maintenant une SORTIE (variable selon Frequence).
                    MAX_SAMPLES_LIMIT = 50000  # limite mémoire sécurité PicoScope

                    if Num_resolution == 12 or Num_resolution == 16:
                        if (duration / num_samples) <= 8e-9:
                            # branche log2 : fréquence d'horloge 500 MHz
                            timebase = round(
                                np.log(500_000_000 / (Frequence * P)) / np.log(2) + 1)
                            dt = 2 ** (timebase - 1) / 500_000_000
                        else:
                            # branche linéaire 62.5 MHz (timebase ≥ 3)
                            timebase = max(3, round(62_500_000 / (Frequence * P) + 3))
                            dt = (timebase - 3) / 62_500_000
                        samples_per_period = round(1.0 / (Frequence * dt))
                        maxSamples = Nbre_periode * samples_per_period
                        if maxSamples % 2 != 0:
                            maxSamples -= 1
                            samples_per_period = maxSamples // Nbre_periode
                        ...
                        if maxSamples % 2 != 0:
                            maxSamples -= 1
                            samples_per_period = maxSamples // Nbre_periode
                    else:  # 14 bits — horloge 125 MHz, timebase min = 3 (Δt = 8 ns)
                        timebase = max(3, round(125_000_000 / (Frequence * P) + 2))
                        ...
                        if Numero_Channel[0] == "PS5000A_CHANNEL_A":
                            print(f"[DEBUG acq] Freq={Frequence:.1f}Hz timebase={timebase} "
                    ...
                    if maxSamples > MAX_SAMPLES_LIMIT:
                        print(f"[WARNING acq] maxSamples={maxSamples} dépasse la limite "
                              f"{MAX_SAMPLES_LIMIT}, bornage en cours")
                        maxSamples = MAX_SAMPLES_LIMIT - (MAX_SAMPLES_LIMIT % Nbre_periode)
                        samples_per_period = maxSamples // Nbre_periode
        ...
        def Asservissement(Mode_asservissement, ChampBdes, ChampB, derivChampBdes, derivChampB, entree, timeC, f_0, Mat_Broyden, reinit):
            ...
            if Mode_asservissement == 'PI FFT':
                ...
                # CONV4 : protection division par zéro sur les normalisations
                ...
                if Forme == 'Sinusoïdale B':
                    if (THDN(derivChampB, num_samples) > 8 or np.max(ChampB) > 1.5*Ampli or np.max(derivChampB) > np.max(derivChampBdes)*1.5 or RMSE > np.min(RMSE_memoire)*1.10) and cycle > 2:
                        print("Back-Tracking (sinusoïdal)")
                    ...
                    if (THDN(derivChampB, num_samples) > 30 or np.max(ChampB) > 1.2*Ampli or np.max(derivChampB) > np.max(derivChampBdes)*1.2 or RMSE > np.min(RMSE_memoire)*1.10) and cycle > 2:
                        print("Back-Tracking (triangulaire)")
                        indice = np.argmin(RMSE_memoire)
                        RMSE_min = RMSE_memoire[indice]
                        while len(RMSE_memoire) != 0:
                            RMSE_memoire.pop(-1)
                ...
                # CONV4 : protection division par zéro (Quasi-Newton)
        ...
        def filtre(signal_a_filtrer):
            ...
            while RMSE>0.8*RMSE_min and RMSE<1.2*RMSE_min:
                ...
                derivchampB4 = savgol_filter(derivchampB, window_length=fenetre, polyorder=3, mode="nearest")
                ...
                if RMSE < RMSE_min:
                    ...
                    fenetre_opt = fenetre
                fenetre += 50#2 
            derivchampB4=savgol_filter(derivchampB, window_length=fenetre_opt, polyorder=3, mode=mode_filtre)
        ...
        def lecture(port_connection):
            ...
            with serial.Serial(port_connection, baudrate=9600, timeout=1) as ser:
        ...
        def Connection():
            ...
            for port in serial_port: # on parcours tous les ports
                if port.device[:-1]=="/dev/ttyUSB": # on récupère l'amplificateur branché sur le port USB
                    # VÉRIF3 : utilisation de 'with' pour garantir la fermeture du port série
        ...
        # Calcul anticipé de samples_per_period avec la MÊME formule quantifiée que Mesure(),
        # afin que les signaux initiaux (entree, ChampBdes…) soient déjà sur la bonne grille.
        _P = num_samples // Nbre_periode
        if Num_resolution == 12 or Num_resolution == 16:
            if (Nbre_periode / (Frequence * num_samples)) <= 8e-9:
                _tb = round(np.log(500_000_000 / (Frequence * _P)) / np.log(2) + 1)
                _dt = 2 ** (_tb - 1) / 500_000_000
            else:
                _tb = max(3, round(62_500_000 / (Frequence * _P) + 3))
                _dt = (_tb - 3) / 62_500_000
        elif Num_resolution == 8:
            _dt = 2e-9
        else:  # 14 bits
            _tb = max(3, round(125_000_000 / (Frequence * _P) + 2))
            _dt = (_tb - 2) / 125_000_000
        samples_per_period_init = round(1.0 / (Frequence * _dt))

        if USE_PROBE_D and int(Num_resolution) > 14:
            raise RuntimeError("4 voies actives : le PS5000A impose une résolution <= 14 bits.")
        ...
        # On NE touche PAS au global Sonde : il doit rester le label ("1", "1/10"...)
        # pour que le combo de configuration le retrouve, sinon il retombe sur "1".
        sonde_str = str(Sonde)
        if   sonde_str == "1":     sonde_gain = 1.0
            ...
            try:
                ...
                sonde_gain = 1.0
        print(f"[SONDE] label={sonde_str!r} -> gain de recuperation = {sonde_gain}")
        ...
        # injecté par l'amplificateur. Fallbacks à 0.0 OBLIGATOIRES : idc_shunt /
        # idc_probe en dépendent à chaque itération (NameError sinon).
        offset_ih_shunt = 0.0
        offset_ih_probe = 0.0
        ...
        if MESURE_OFFSETS_AU_DEMARRAGE:
            try:
                ...
                offsets_mesures = True
                print(f"[OFFSET] shunt={offset_ih_shunt*1e3:.2f} mA  "
                      f"sonde={offset_ih_probe*1e3:.2f} mA (excitation OFF)")
            except Exception as e:
                print("[OFFSET] mesure des offsets impossible :", e)
        try:
            ...
            print("[OFFSET] écriture metadata impossible :", e)
        ...
        try:
            while True:
                ...
                if self.Warnig == '1':
                    print("[SECURITE] defaut amplificateur — arret de la mesure.")
                ...
                integral = integrate.cumtrapz(v2_for_integration, timeC, initial=0)
                ...
                ChampB = integral_corrige/(-N2*Section*facteur_mm2_vers_m2)  # Conversion de la section de mm² en m² pour obtenir B en Tesla
                #print("champB=")
               ...
               # for val in ChampB:
                ...
                def filtre_harmonique(sig, n_harm=15):
                    X  = np.fft.rfft(sig)
                    Xf = np.zeros_like(X)
                    Xf[1:n_harm+1] = X[1:n_harm+1]     # fondamental + 15 harmoniques, continu retiré
                    return np.fft.irfft(Xf, n=len(sig))
                
                ChampH2 = filtre_harmonique(ChampH23, n_harm=15)
                if USE_PROBE_D:
                    ...
                    ChampH2_probe  = filtre_harmonique(ChampH23_probe, n_harm=15)
                    ...
                    print(f"[SONDE-D] |Ip|/|Ish|={ratio_I:.4f}  Δφ={dphi_deg:+.3f}°  "
                ...
                ff_val = FF(derivChampB, timeC[samples_per_period])
                ff_des = FF(derivChampBdes, timeC[samples_per_period])
                ...
                converge = ((Amplitude*1.005 > (np.max(np.abs(ChampB)))) & (Amplitude*0.995 < (np.max(np.abs(ChampB)))) & (THDN(derivChampB, num_samples) < THDN(derivChampBdes, num_samples)+3) & ((FF(derivChampB, timeC[samples_per_period]) < FF(derivChampBdes, timeC[samples_per_period])*1.01) & (FF(derivChampB, timeC[samples_per_period]) > FF(derivChampBdes, timeC[samples_per_period])*0.99)))
            ...
            Pv = np.trapz(ChampH2[sl] * derivChampB[sl],
            ...
            if Pv<0 : # dans le cas de faible signaux, on peut avoir le mauvais déphasage (déphasé de pi). 
                ...
                Pv*=-1
                print("les pertes inverse")
        
            mu_r = max(ChampB)/(max(ChampH2)*mu_0)  # Calcul de la perméabilité relative, mu_0 = 4π×10⁻⁷ H/m
            Pv_probe = None
            if USE_PROBE_D:
                Pv_probe = np.trapz(ChampH2_probe[sl] * derivChampB[sl],
                                    x=timeC[:samples_per_period]) / (timeC[samples_per_period])
                if Pv < 0:                      # garder les deux dans la même convention de signe
                    Pv_probe *= -1
                ecart = 100.0 * (Pv_probe - abs(Pv)) / max(abs(Pv), 1e-12)
                print(f"[PERTES] shunt={abs(Pv):.1f} W/m³   sonde={Pv_probe:.1f} W/m³   écart={ecart:+.1f}%")
            ...
            try:
                ...
                self.parent.main_window.update_mesures(
                    np.max(ChampB), Frequence, Hc, Br, np.max(ChampH2), mu_r, Pv, Pv_probe)

            except Exception as e:   # VÉRIF2 : réactivé avec gestion d'erreur propre
                print('erreur (post-boucle) : ', e)
                Erreur.append(str(e))
...
class ConfigWindow(QDialog, Ui_Dialog):
    def __init__(self, *args, parent=None, **kwargs):
        ...
        # Shunt mesuré 4 fils = 0.101 Ω, valeur figée au point d'affectation
        self.Rh_spin.setValue(0.101)
    ...
    def Calcul_R_N1(self): 
        ...
        H=Ampli/(mu_r*mu_0)  # Calcul du champ H à partir de B désiré et de mu_0 (4π×10⁻⁷ H/m)
    ...
    def accept(self):
        ...
        print(f"[SONDE][accept] combo lu = {Sonde!r}")
    ...
    def __init__(self, main_window, *args, obj=None, **kwargs):
        ...
        if Mode_Auto=="Simple":
            ...
            if self.thread is None or not self.thread.isRunning():
                ...
                self.thread.update_plot.connect(self.on_update_plot)   # BUG2
                self.thread.start()
                self.show()
...
class DemagWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # FIX-DEMAG : Signal de démagnetisation amélioré
            # Problème ancien : truncation à la moitié (2.5s) → amplitude résiduelle ~8%
            # → échantillon pas complètement démagnétisé.
            # Fix : utiliser le signal complet (5s) avec decay exp(-1.5*t) pour atteindre
            # <0.1% en ~5s (e^(-1.5*5) ≈ 0.05%), et 5000 points pour un signal propre.
            a, b = 0, 5
            t = np.linspace(a, b, num=5000)
            entree = np.exp(-1.5 * t) * np.cos(2 * np.pi * t * 50)
            # Pas de truncation : on utilise le signal complet
            # Ajouter des zéros à la fin pour assurer un retour propre à 0
            dt = t[1] - t[0]
...
class MainWindow(QtWidgets.QMainWindow, Ui_JTcontrol):
    ...
    def plot_data(self, x, y):
        if x is None or y is None:
            print("[plot_data] x ou y est None — rien à tracer (mesure interrompue ?)")
            return
    ...
    def Verification_Erreur(self):
        ...
        # =========================================================================
        print("\n" + "="*80)
        print("VERIFICATION PARAMÈTRES - Affichage des valeurs saisies")
        print("="*80)
        ...
        for i in range(len(Param)):
            ...
            print("{:<20} {:<15.6g} {:<10}".format(Param_str[i], Param[i], unit_str))
        ...
        print("\n" + "-"*80)
        print("VÉRIFICATION 1: Paramètres manquants (= 0)")
        print("-"*80)
        ...
        for i in range(len(Param)):
            if Param[i] == 0:
                ...
                print(f"❌ {Param_str[i]} = 0 → ERREUR")
        
        if not param_zero:
            print("✓ Tous les paramètres sont renseignés")
        
        # =========================================================================
        # ÉTAPE 3: VÉRIFICATION - COHÉRENCE GÉOMÉTRIQUE
        # =========================================================================
        print("\n" + "-"*80)
        print("VÉRIFICATION 2: Cohérence géométrique")
        print("-"*80)
        print(f"Di (diamètre intérieur) = {Di} mm")
        print(f"De (diamètre extérieur) = {De} mm")
        
        if De < Di:
            Erreur.append("De < Di (géométrie impossible)")
            print(f"❌ ERROR: De ({De}) < Di ({Di})")
            print(f"   → Le diamètre extérieur doit être > diamètre intérieur !")
        else:
            diam_diff = De - Di
            print(f"✓ De > Di (épaisseur tore = {diam_diff} mm)")
        
        # =========================================================================
        # ÉTAPE 4: CONVERSION SONDE
        # =========================================================================
        print("\n" + "-"*80)
        print("VÉRIFICATION 3: Conversion facteur Sonde")
        print("-"*80)
        ...
        if Sonde == "1":
            sonde_val = 1.0
            print(f"Sonde input: '{Sonde_original}' → gain = {sonde_val} (pas d'atténuation)")
        elif Sonde == "1/10":
            sonde_val = 10.0
            print(f"Sonde input: '{Sonde_original}' → gain = {sonde_val} (1/10, ÷10 → ×10)")
        elif Sonde == "1/100":
            sonde_val = 100.0
            print(f"Sonde input: '{Sonde_original}' → gain = {sonde_val} (1/100, ÷100 → ×100)")
        else:
            try:
                v = float(Sonde)
                sonde_val = (1.0 / v) if 0.0 < v < 1.0 else v
                print(f"Sonde input: '{Sonde_original}' → gain = {sonde_val} (nombre)")
            except Exception:
                sonde_val = 1.0
                Erreur.append(f"Sonde invalide: '{Sonde_original}'")
                print(f"❌ ERROR: Sonde '{Sonde_original}' non reconnue → gain = 1.0")
        
        # =========================================================================
        # ÉTAPE 5: CALCUL ET VÉRIFICATION DE LA TENSION DU SECONDAIRE
        # =========================================================================
        print("\n" + "-"*80)
        print("VÉRIFICATION 4: Limite de tension du secondaire V₂")
        print("-"*80)
        ...
        # Calcul de V₂
        # V₂ = 4.44 × Ns2 × Section(m²) × Ampli × Freq × √2 × Sonde
        V2 = K_faraday * Ns2 * Section_m2 * Ampli * Freq * sqrt_2 * sonde_val
        
        # Affichage de la formule et du calcul
        print(f"\nFormule de Faraday:")
        print(f"V₂ = K × Ns2 × S(m²) × Ampli × Freq × √2 × Sonde")
        print(f"\nOù:")
        print(f"  K (Faraday)     = {K_faraday}")
        print(f"  Ns2 (spires)    = {Ns2}")
        print(f"  Section (mm²)   = {Section}")
        print(f"  Section (m²)    = {Section_m2:.10e}")
        print(f"  Ampli (V)       = {Ampli}")
        print(f"  Freq (Hz)       = {Freq}")
        print(f"  √2              = {sqrt_2:.6f}")
        print(f"  Sonde (gain)    = {sonde_val}")
        
        print(f"\nCalcul pas à pas:")
        print(f"  V₂ = {K_faraday} × {Ns2} × {Section_m2:.10e} × {Ampli} × {Freq} × {sqrt_2:.6f} × {sonde_val}")
        
        # Calculs intermédiaires pour montrer l'ordre de grandeur
        step1 = K_faraday * Ns2
        step2 = step1 * Section_m2
        step3 = step2 * Ampli
        step4 = step3 * Freq
        step5 = step4 * sqrt_2
        step6 = step5 * sonde_val
        
        print(f"    = {step1:.4f} (après K × Ns2)")
        print(f"    = {step2:.10e} (après × Section)")
        print(f"    = {step3:.10e} (après × Ampli)")
        print(f"    = {step4:.6f} (après × Freq)")
        print(f"    = {step5:.6f} (après × √2)")
        print(f"    = {V2:.6f} V (après × Sonde) ← RÉSULTAT FINAL")
        
        # Vérification de la limite
        V2_limit = 20.0
        print(f"\nVérification:")
        print(f"  V₂ calculé = {V2:.6f} V")
        print(f"  V₂ limite  = {V2_limit} V (limite picoscope)")
        
        if V2 >= V2_limit:
            Erreur.append(f"Limite de tension atteinte (V₂ = {V2:.3f} V ≥ {V2_limit} V)")
            print(f"❌ ERROR: V₂ = {V2:.3f} V >= {V2_limit} V")
            print(f"   → Le picoscope ne peut pas mesurer plus de {V2_limit} V !")
            print(f"   → Solutions:")
            print(f"      • Réduire Ampli (actuellement {Ampli} V)")
            print(f"      • Augmenter Sonde (actuellement {Sonde})")
            print(f"      • Réduire Ns2 (actuellement {Ns2} spires)")
            print(f"      • Réduire Freq (actuellement {Freq} Hz)")
            print(f"      • Réduire Section (actuellement {Section} mm²)")
        else:
            margin = (1 - V2/V2_limit) * 100
            print(f"✓ V₂ = {V2:.3f} V < {V2_limit} V")
            print(f"   Marge de sécurité: {margin:.1f}%")
    ...
    def closeEvent(self, event):
        ...
        """
        # 1) Arrête le thread Worker d'asservissement s'il existe (via SupervisionWindow)
        if hasattr(self, 'w') and self.w is not None:
            try:
                self.w.stop = True  # drapeau coopératif lu dans Worker.run()
                if hasattr(self.w, 'thread') and self.w.thread is not None:
                    if self.w.thread.isRunning():
                        print("⏳ Arrêt du thread d'asservissement...")
                        if not self.w.thread.wait(5000):  # 5 s max
                            print("⚠️ Asservissement non terminé, arrêt forcé.")
                            self.w.thread.terminate()
                            self.w.thread.wait()
            except Exception as e:
                print(f"Erreur arrêt asservissement : {e}")
        
        # 2) Arrête le thread Temperature_Ampli si encore vivant
        if hasattr(self, 'thread') and self.thread is not None:
            try:
                if self.thread.isRunning():
                    print("⏳ Arrêt du thread Temperature_Ampli...")
                    self.thread.quit()
                    if not self.thread.wait(3000):
                        self.thread.terminate()
                        self.thread.wait()
            except Exception as e:
                print(f"Erreur arrêt Temperature : {e}")
        ...
        if hasattr(self, '_demag_thread') and self._demag_thread is not None:
            try:
                if self._demag_thread.isRunning():
                    print("⏳ Arrêt du thread DemagWorker...")
