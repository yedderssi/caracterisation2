...
class MeasurementLogger:
    ...
    def __init__(self, metadata, root_dir="./Logs"):
        ...
        nom_session = (f"{timestamp}_"
                       ...
                       f"B{metadata.get('Nm_ref', 'inconnu')}")
        ...
        print(f"📁 Logger initialisé : {self.dir}")
        ...
        try:
            while True:
               ...
               # for val in ChampB:
                ...
                print(f"[DEBUG iter] Freq={Frequence:.1f}Hz spp={samples_per_period} "
                      f"len(entree)={len(entree)} len(ChampBdes)={len(ChampBdes)} "
                      f"len(ChampB_spp)={len(ChampB[:samples_per_period])} "
                      f"len(dChampBdes)={len(derivChampBdes)} "
                      f"len(dChampB_spp)={len(derivChampB[:samples_per_period])}")
