...
class Worker(QThread):
    ...
    def run(self):
        """
        Permet de mesurer les signaux et calculer les valeurs intrasec de l'élément mesuré.
        ...
        def compute_fft_signals(B_ref_sim, B_sim, V1_sim, dB_dt_ref_sim, dB_dt_sim):
            ...
            an_dB_ref = 2*np.real(np.fft.rfft(dB_dt_ref_sim)
                                  )/len(dB_dt_ref_sim)
            bn_dB_ref = -2 * \
                np.imag(np.fft.rfft(dB_dt_ref_sim))/len(dB_dt_ref_sim)
        ...
        def GBF(entree):
            ...
            instr.write(
                f'APPLy:ARB {len(entree)*Frequence},{pk},{np.mean(entree)}')
            instr.write("OUTP ON")
            rawError = ''
            errorCode = -1
            while errorCode != 0:
        ...
        # Selection[i]= Channel
        # Selection[i][u] = paramètre du channel i
        Selection_Channel = [["PS5000A_CHANNEL_A", "setChA", "setdataBuffersA"],
                             ["PS5000A_CHANNEL_B", "setChB", "setdataBuffersB"],
                             ["PS5000A_CHANNEL_C", "setChC", "setdataBuffersC"]]

        Range=[] # Initialisation du tableau contenant la plage de mesure
        
        iteration = 0
        Nbre_enregistrement = Nbre_enregist # Similaire à la foncrion moyennage des oscillo traditionnels
        ...
        Rsh = Rh
        w = Frequence*2*np.pi
        num_samples = int(num_samples) # Choix du nombre d'échantillon pour notre acquisition
        ...
        entree, pk = GBF(entree)
        # Auto Range
        Range=Auto_Range(entree)
        for i in range(len(Range)):
            Selection_Channel[i].append(Range[i]) # ajout de la plage de mesure dans Selection_Channel
        print(Selection_Channel)  
        ...
        while True:
            ...
            BufferMax, cmaxSamples, timeIntervalns = PicoScope(
            ...
            # Supression de la compoasnte continue
            ...
            # Deuxieme tableau pour avoir l'axe des temps
            timeC = np.arange(
                np.array(samples_to_plotC).shape[0]) * timeIntervalns.value/1e9


            # Calcule du champs B
            Signal_sans_moyenne = detrend(samples_to_plotC, type='constant') # Supression de la composante continue
            integral = integrate.cumulative_trapezoid(
                (Signal_sans_moyenne)/1000, timeC, initial=0) 
            derivChampBdes = f2(timeC[:samples_per_period]) # récupération de la fonction interpolée f2 calculé en début de PRGM,
                                                            # On adampte notre dérivée à l'axe des temps
            ChampBdes = f3(timeC[:samples_per_period])
            Maxi = np.max(integral)
            Mini = np.min(integral)
            integral_corrige = integral-((Mini+Maxi)/2) # Supression de la composante continue (Une deuxième fois ???)
            ...
            if len(cross) == 2:
                ...
                #     lag = -cross[1]
                lag = np.where(derivChampB == np.max(
                    derivChampB[:samples_per_period]))[0][0]
            ...
            Fourier_e_k = Fourier_V + \
                (- alpha/5 * (Fourier_B_ref-Fourier_B) -
                 beta/5 * (Fourier_dB_ref-Fourier_dB))
            print('iter=', iteration, 'Amp_V=', round(np.max(entree), 3), 'delta_B=', round(np.max((Fourier_B_ref-Fourier_B) /
                  np.max(Fourier_B_ref)), 3), 'delta_dB=', round(np.max((Fourier_dB_ref-Fourier_dB)/np.max(Fourier_dB_ref)), 3))
            e_k = 0.5*len(entree)*np.fft.irfft(Fourier_e_k[0:int(
                len(Fourier_e_k)/2)] - 1j * Fourier_e_k[int(len(Fourier_e_k)/2):])
            entree = e_k

            entree = savgol_filter(
                entree, window_length=100, polyorder=3, mode="wrap")
            ChampH2 = savgol_filter(
                ChampH2, window_length=100, polyorder=3, mode="wrap")
