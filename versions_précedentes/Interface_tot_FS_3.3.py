...
Nbre_periode = 2
iteration_max = 50
num_samples = 2000  #Nombre de points par periode
...
class Worker(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.parent = parent
    def run(self):
        global Materiaux, Hauteur, Di, De, Section, Outils, Ns1, Ns2, Rs, Rh, Type, Forme, Freq, Ampli, Gain, Nbre_periode, Kf, Nm_ref, mu_r, lm, num_samples,Resolution,Nbre_enregist,alpha,beta,iteration_max, num_Fourier, Mat_Broyden, f_0
        
        import ctypes
        import pyvisa as visa,usbtmc
        import time as tm
        ...
        if Forme =='Sinusoïdale B':
            ...
            ChampBdes = Amplitude*sawtooth(w*t,0.5)
        ...
        while True:
            ...
            for nbre in entree:
                message2 = message2 + ', ' + str("%.3f" % round(nbre/np.max(entree),3))
                index+=1
            instr.timeout = 5000
            ...
            while True:
                ...
                #chCRange = ps.PS5000A_RANGE["PS5000A_20V"]
                # analogue offset = 0 V
                status["setChA"] = ps.ps5000aSetChannel(
            ...
            derivChampB = samples_to_plotC/(-N2*Section*1e-6*1000)

            # ChampH1 = ChampB/(Mu*mu_0)
            ...
            # self.parent.widget_3.canvas.ax.plot(timeC[:samples_per_period],derivChampB[:samples_per_period]*(-N2*Section*1e-6),label="V2 avant synchro")
            # ChampBdes_analytic = hilbert(ChampBdes)
            # ChampB_analytic = hilbert(ChampB[:samples_per_period])
            # phase_diff_Bdes = np.angle(ChampBdes_analytic) - np.angle(ChampB_analytic) 

            # ChampB_aligne = np.abs(ChampB_analytic) * np.exp(1j * (np.angle(ChampB_analytic) + phase_diff_Bdes))
            # ChampB2 = np.real(ChampB_aligne)
            # signal_correle = np.correlate(
            #     ChampB2, ChampB[:samples_per_period], "same")
            # lags = correlation_lags(
            #     ChampB2.size, ChampB[:samples_per_period].size, mode="same")
            # lag = lags[np.argmax(signal_correle)]
            # ChampB = ChampB2
            # derivChampB_analytic = hilbert(derivChampB[:samples_per_period])
            # derivChampB_aligne = np.abs(derivChampB_analytic) * np.exp(1j * (np.angle(derivChampB_analytic) + phase_diff_Bdes))
            # derivChampB = np.real(derivChampB_aligne)
            
            # ChampH2_analytic = hilbert(ChampH2[:samples_per_period])
            # ChampH2_aligne = np.abs(ChampH2_analytic) * np.exp(1j * (np.angle(ChampH2_analytic) + phase_diff_Bdes + np.pi))
            # ChampH2 = np.real(ChampH2_aligne)
            
            # signalA = samples_to_plotA[:samples_per_period]/50
            ...
            # derivChampB = savgol_filter(derivChampB, window_length=200, polyorder=3, mode="wrap")
            signal_correle = np.correlate(
            ...
            self.parent.widget_3.canvas.ax.plot(timeC[:samples_per_period],derivChampBdes*(-N2*Section*1e-6),label="V2 desiré",color='r',linestyle='--')
            ...
            line3, = self.parent.widget_3.canvas.ax.plot(timeC[:samples_per_period],derivChampB[:samples_per_period]*(-N2*Section*1e-6),label="V2")
            ...
            THD_dB, Fourier_dB_ref, Fourier_dB,Fourier_B_ref, Fourier_B, Fourier_V = compute_fft_signals(ChampBdes,ChampB[:samples_per_period],entree,derivChampBdes,derivChampB[:samples_per_period])
            RMSE, FF, err_dB_amp = compute_convergence_criteria(derivChampBdes,derivChampB[:samples_per_period],ChampBdes,ChampB[:samples_per_period],timeC[:samples_per_period])
          
            
            if Mode_asservissement == 'PI_FFT':
                Fourier_e_k= (Fourier_V  - 0.5 * (Fourier_B_ref-Fourier_B)/np.max(Fourier_B_ref) - 0.5 * (Fourier_dB_ref-Fourier_dB)/np.max(Fourier_dB_ref))
            ...
            #RMSE = math.sqrt(np.square(np.subtract(derivChampB[:samples_per_period],derivChampBdes)/np.max(derivChampBdes)).mean())
            ...
            self.parent.FF_value.setText(str(round(FF,3))+" %")
            self.parent.THD_value.setText(str(round(THD_dB, 3))+" %")
            self.parent.RMSE_value.setText(str(round(RMSE,3))+" %")
            self.parent.Hmax_value.setText(str(round(np.max(ChampH2),2)))
            self.parent.Imax_value.setText(str(round(np.max(ih),3)))
            # if iteration == 10:
            # if (THD_dB<1):
            #     self.parent.THD_value.setStyleSheet("Qlabel{color: green;}")
            # else:
            #     self.parent.THD_value.setStyleSheet("Qlabel{color: black;}")
            # if (Amplitude<(np.max(ChampB)*1.005)) & (Amplitude>(np.max(ChampB)*0.995)):
            #     #self.parent.Bkmax_value.setStyleSheet("Qlabel{color: green;}")
            #     self.parent.Bkmax_value.setStyleSheet("Qlabel{color: green;}")
            # else:
            #     #self.parent.Bkmax_value.setStyleSheet("Qlabel{color: black;}")
            #     self.parent.Bkmax_value.setStyleSheet("Qlabel{color: black;}")
            # if (FF < 1):
            #     self.parent.FF_value.setStyleSheet("Qlabel{color: green;}")
            # else:
            #     self.parent.FF_value.setStyleSheet("Qlabel{color: black;}")
            #self.parent.iteration_label.setStyleSheet("Qlabel{color: black;}")
            if ((Amplitude<(np.max(ChampB)*1.005)) & (Amplitude>(np.max(ChampB)*0.995))& (THD_dB<2) & (FF < 0.5)) or iteration==iteration_max or self.parent.stop==True:
                self.parent.stop == False
                self.parent.stop_button.setEnabled(True)
                #self.parent.iteration_label.setStyleSheet("Qlabel{color: red;}")
                break
            iteration += 1
        ...
        Br = np.mean(Brs)
        Pv = np.trapz(ChampH2[:samples_per_period] * derivChampB[:samples_per_period], x=timeC[:samples_per_period])/ timeC[samples_per_period]
        ...
        self.parent.main_window.update_mesures(np.max(ChampB), Frequence, Hc, Br, np.max(ChampH2), mu_r, Pv)
        self.finished.emit()
...
class ComparWindow(QDialog, Ui_curve_comparaison):
    ...
    def export_table(self):
        ...
        filename, _ = QtWidgets.QFileDialog.getSaveFileName(None, "Export Table to CSV", "Comparaison de courbes.csv", "CSV Files (*.csv)")
        
        if filename:
            with open(filename, 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)

                header_data = [model.headerData(col, Qt.Horizontal) for col in range(columns)]
    ...
    def load_csv_data(self, file_path):
        ...
        self.tableWidget.setItem(empty_row, 1, QTableWidgetItem(str(round(float(bmax),3))))
        self.tableWidget.setItem(empty_row, 2, QTableWidgetItem(freq))
        self.tableWidget.setItem(empty_row, 3, QTableWidgetItem(str(round(float(hc),3))))
        self.tableWidget.setItem(empty_row, 4, QTableWidgetItem(str(round(float(br),3))))
        self.tableWidget.setItem(empty_row, 5, QTableWidgetItem(str(round(float(hmax),3))))
        self.tableWidget.setItem(empty_row, 6, QTableWidgetItem(Mu_r))
        self.tableWidget.setItem(empty_row, 7, QTableWidgetItem(str(round(float(W),3))))
        self.tableWidget.setItem(empty_row, 8, QTableWidgetItem(str(round(float(Pv),3))))
...
class ConfigWindow(QDialog, Ui_Dialog):
    ...
    def accept(self):
        ...
        main_window.update_data(Outils, Materiaux, Di, De, Hauteur, Section, lm, Kf)
        self.close()
...
class MainWindow(QtWidgets.QMainWindow, Ui_JTcontrol):
    ...
    def opens_config(self):
        global Materiaux, Hauteur, Di, De, Section, Outils, Ns1, Ns2, Rs, Rh, Type, Forme, Freq, Ampli, Gain, Nbre_periode, Nbre_enregist, Nm_ref, mu_r, Kf, num_samples,alpha,beta
