...
class Worker(QThread):
    ...
    def run(self):
        ...
        def Visualisation(entree):
            ...
            #adc2mVChCMax = np.convolve(adc2mVChCMax, np.ones(100), "valid") / 100
            
            # fc=1000
            # f_nyq=1/((1/Frequence)/len(adc2mVChCMax))/2
            # b, a = signal.butter(4, fc/f_nyq, 'low', analog=False)
            # #Application du filtre
            # adc2mVChCMax = signal.filtfilt(b, a, adc2mVChCMax)
            
            # fft_voie_C = fft(adc2mVChCMax)
            # # plt.stem(fft_voie_C, markerfmt=" ", basefmt="-r")
            # # plt.show()
            # # recherche du fondamentale pour obtenir deux signaux simples
            # voie_C_harmonique = np.zeros(len(fft_voie_C))
            
            # index=0
            # i=0
            # while index!=10:
            #     voie_C_harmonique[i]=np.abs(fft_voie_C[i])
            #     if np.abs(fft_voie_C[i])>1:
            #         index+=1
            #     i+=1
            #     if i ==len(fft_voie_C):
            #         break            
            ...
            # # Transformée de Fourier inverse
            # adc2mVChCMax = ifft(
            #     voie_C_harmonique*np.exp(1j*np.angle(fft_voie_C))).real
            adc2mVChCMax = savgol_filter(adc2mVChCMax, window_length=500, polyorder=3, mode="wrap")
   #
            ...
            ChampH2 = savgol_filter(ChampH2, window_length=500, polyorder=3, mode="wrap")
        ...
        while True:
            ...
            #adc2mVChCMax = np.convolve(adc2mVChCMax, np.ones(400), "valid") / 400
            ...
            # while index!=3:
            #     voie_C_harmonique[i]=np.abs(fft_voie_C[i])
            #     if np.abs(fft_voie_C[i])>10:
            #         index+=1
            #     i+=1
            #     if i ==len(fft_voie_C):
            #         break
            # print("nb harmonique : ",index)
            # print("taille harmonique voie C",len(voie_C_harmonique))
            ...
            print ('Tension V1 : ',np.max(adc2mVChAMax))

            print ('Tension V2 : ',np.max(adc2mVChCMax))
            ...
            if ((Amplitude < (np.max(np.abs(ChampB))*1.01)) & (Amplitude > (np.max(ChampB)*0.99)) & (THDN(derivChampB, num_samples) < THDN(derivChampBdes, num_samples)+3) & ((FF(derivChampB, timeC[samples_per_period]) < FF(derivChampBdes, timeC[samples_per_period])*1.01) & (FF(derivChampB, timeC[samples_per_period]) > FF(derivChampBdes, timeC[samples_per_period])*0.99))) or iteration == iteration_max or self.parent.stop == True:
                
                #ChampB,ChampH2, derivChampB =Visualisation(entree)
                ChampB=ChampB_tempo
                derivChampB=derivChampB_tempo
                ChampH2=ChampH_tempo
                ChampH2 = savgol_filter(ChampH2, window_length=100, polyorder=3, mode="wrap")
                # ChampB = savgol_filter(ChampB, window_length=500, polyorder=3, mode="wrap")
                # derivChampB = savgol_filter(derivChampB, window_length=500, polyorder=3, mode="wrap")
        ...
        indice_sommet=np.argmax(derivChampB)


        Pv_ascendant= np.trapz(ChampH2[:samples_per_period//2] * derivChampB[:samples_per_period//2],
                      x=timeC[:samples_per_period//2]) / (timeC[samples_per_period])
        Pv_ascendant+= np.trapz(-ChampH2[:samples_per_period//2] * -derivChampB[:samples_per_period//2],
                      x=timeC[:samples_per_period//2]) / (timeC[samples_per_period])
        print('Pv ascendant 1 : ',Pv_ascendant)
        
        Pv_ascendant= np.trapz(ChampH2[(indice_sommet-samples_per_period//2):indice_sommet] * derivChampB[(indice_sommet-samples_per_period//2):indice_sommet],
                      x=timeC[(indice_sommet-samples_per_period//2):indice_sommet]) / (timeC[samples_per_period])
        Pv_ascendant+= np.trapz(-ChampH2[(indice_sommet-samples_per_period//2):indice_sommet] * -derivChampB[(indice_sommet-samples_per_period//2):indice_sommet],
                      x=timeC[(indice_sommet-samples_per_period//2):indice_sommet]) / (timeC[samples_per_period])
        print('Pv ascendant 2 : ',Pv_ascendant)

        Pv_descendant= np.trapz(ChampH2[indice_sommet:(indice_sommet+samples_per_period//2)] * derivChampB[indice_sommet:(indice_sommet+samples_per_period//2)],
                      x=timeC[indice_sommet:(indice_sommet+samples_per_period//2)]) / (timeC[samples_per_period])
        Pv_descendant+= np.trapz(-ChampH2[indice_sommet:(indice_sommet+samples_per_period//2)] * -derivChampB[indice_sommet:(indice_sommet+samples_per_period//2)],
                      x=timeC[indice_sommet:(indice_sommet+samples_per_period//2)]) / (timeC[samples_per_period])
        print('Pv descendant 1 : ',Pv_descendant)
        
        Pv_descendant= np.trapz(ChampH2[samples_per_period//2:samples_per_period] * derivChampB[samples_per_period//2:samples_per_period],
                      x=timeC[samples_per_period//2:samples_per_period]) / (timeC[samples_per_period])
        Pv_descendant+= np.trapz(-ChampH2[samples_per_period//2:samples_per_period] * -derivChampB[samples_per_period//2:samples_per_period],
                      x=timeC[samples_per_period//2:samples_per_period]) / (timeC[samples_per_period])
        print('Pv descendant 2 : ',Pv_descendant)
