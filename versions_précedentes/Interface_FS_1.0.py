...
class Worker(QThread):
    ...
    def run(self):
        """
        Permet de mesurer les signaux et calculer les valeurs intrinseques de l'élément mesuré.
        ...
        def compute_fft_signals(B_des, B_reel, V1_sim, dB_des, dB_reel):
            """
            Cette fonction calcule la transformée de Fourier des signaux mesurés
            """
        ...
        while True:
          ...
          #  plt.plot(timeC[:samples_per_period], ChampBdes, '-')
            ...
            for val in ChampB:
                  print(val)
            derivChampB = samples_to_plotC / (-N2*Section*1000)  # Sample_to_Plot_C = U2
...
class ConfigWindow(QDialog, Ui_Dialog):
    ...
    def calcul_kf(self, var):
        ...
        Di = self.Di_spin.value()*1e-3#Les données sont rentrées en mm, mais on sauvegarde en m
        De = self.De_spin.value()*1e-3#Les données sont rentrées en mm, mais on sauvegarde en m
        Hauteur = self.Hauteur_spin.value()*1e-3
            
        if Di != 0 and De != 0 and Hauteur != 0:
            Stheo = Hauteur*(De-Di)/2
            Kf = var*1e-6/Stheo
            self.Kf_spin.setValue(Kf)
        self.Kf_spin.valueChanged.connect(self.calcul_section)
    ...
    def calcul_longueur(self, var):
        ...
        Di = self.Di_spin.value()*1e-3
        De = self.De_spin.value()*1e-3
        if Di != 0 and De != 0:
