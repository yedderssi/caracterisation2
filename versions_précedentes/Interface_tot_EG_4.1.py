...
class Worker(QThread):
    ...
    def run(self):
        ...
        def PicoScope(Selection_Channel, Channelranges, entree,Range):
        ...
        for i in range(len(Range)):
            Selection_Channel[i].append(Range[i])
        ...
        while True:
            ...
            BufferMax, cmaxSamples, timeIntervalns = PicoScope(
                Selection_Channel, Channel_ranges, entree, Range)

            # Mise en forme sur une période
            BufferMaxAf = np.mean(BufferMax[0], axis=0)

            # plt.plot(BufferMaxAf, 'blue')
            # plt.show()

            BufferMaxBf = np.mean(BufferMax[1], axis=0)

            BufferMaxCf = np.mean(BufferMax[2], axis=0)
