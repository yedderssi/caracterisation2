...
class Worker(QThread):
    ...
    def run(self):
        ...
        def Synchronisation(ChampBdes, ChampB, temps, nb_harmoniques=1):
            """
            Synchronise deux signaux périodiques en utilisant la FFT et la transformée de Hilbert.
            On garde un nombre d'harmoniques fixé pour reconstruire les signaux avant de mesurer le déphasage.

            Parameters
            ----------
            ChampBdes : array_like
                Signal de référence (une période)
            ChampB : array_like
                Signal à synchroniser (une période)
            temps : array_like
                Vecteur temporel correspondant aux signaux
            nb_harmoniques : int
                Nombre d'harmoniques à conserver lors de la reconstruction
            ...
            ChampB : ndarray
              Signal synchronisé
            lag : int
             Décalage appliqué (en indices)
             """
            
            Te = 1e-3
            N = 1000
            temps = np.arange(N) * Te
            # Extraction des harmoniques pour chaque signal
            ChampBdes_prim = extraire_harmoniques(ChampBdes, temps, nb_harmoniques)
            ChampB_prim = extraire_harmoniques(ChampB, temps, nb_harmoniques)

            # Alignement par recherche du déphasage temporel
            try:
                 _, a = start_from_zero(ChampBdes_prim)
                 _, b = start_from_zero(ChampB_prim)
                 lag = b - a
            ...
            # Transformée de Hilbert et correction de phase
            ...
            aligne = np.abs(Hilbert_ChampB) * np.exp(1j * (np.angle(Hilbert_ChampB) + phase_diff))
        ...
        def Asservissement(Mode_asservissement, ChampBdes, ChampB, derivChampBdes, derivChampB, entree, timeC, f_0, Mat_Broyden, reinit):
            ...
            entree, lag = Synchronisation(
                derivChampBdes, entree,1) # synchronisation de l'entree sur le dB pour faciliter la convergence
        ...
        def filtre(signal_a_filtrer, n_harmoniques=None):
            """
            Filtre un signal en choisissant entre filtre FFT ou Savitzky-Golay en fonction de l'erreur RMSE.
    
            Parameters
            ----------
            signal_a_filtrer : ndarray
            Une période du signal à filtrer.
    
            n_harmoniques : int or None
            Nombre d'harmoniques à conserver pour le filtrage FFT.
            Si None, le programme détermine automatiquement l'optimum.
            ...
            # Extraction du fondamental
            fond_signal = np.zeros_like(fft_signal)
            fond_signal[np.argmax(np.abs(fft_signal))] = np.max(np.abs(fft_signal))

            # Signal idéal (base)
            x1_base = 2 * ifft(fond_signal * np.exp(1j * np.angle(fft_signal))).real
            if np.max(x1_base) == 0:
               x1_base = np.ones(len(x1_base))

             # Transformée de Fourier complète
            X1 = fft(signal_a_filtrer)

             # --- Sélection des harmoniques ---
            if n_harmoniques is None:
             # Mode automatique : optimiser le nombre d'harmoniques
               optimal_harmoniques = 0
               min_RMSE = 1e6
               for num_h in range(1, 20):
                   X1_temp = np.zeros_like(X1, dtype=complex)
                   indices = np.argsort(-np.abs(X1))  # tri décroissant des amplitudes
                   for k in indices[:num_h]:
                       X1_temp[k] = X1[k]
               signal_reconstruit = 2 * ifft(X1_temp).real
               delta = (x1_base - signal_reconstruit) / np.max(x1_base)
               RMSE = 100 * np.sqrt(np.mean(delta ** 2))
               if RMSE < min_RMSE:
                  min_RMSE = RMSE
                  optimal_harmoniques = num_h
               n_harmoniques = optimal_harmoniques

            # Application du filtre FFT avec `n_harmoniques`
            X1_filtre = np.zeros_like(X1, dtype=complex)
            indices = np.argsort(-np.abs(X1))
            for k in indices[:n_harmoniques]:
                X1_filtre[k] = X1[k]
            signal_fft = 2 * ifft(X1_filtre).real

            # --- Filtrage Savitzky-Golay ---
            fenetre = 51
            fenetre_opt = fenetre
            RMSE_min = 1e6
            RMSE = 30
            while fenetre <= len(x1_base) and RMSE > 0.8 * RMSE_min and RMSE < 1.2 * RMSE_min:
                 try:
                     signal_savgol = savgol_filter(signal_a_filtrer, window_length=fenetre, polyorder=1, mode=mode_filtre)
                     delta = (x1_base - signal_savgol) / np.max(x1_base)
                     RMSE = 100 * np.sqrt(np.mean(delta ** 2))
                     if RMSE < RMSE_min:
                        RMSE_min = RMSE
                        fenetre_opt = fenetre
            ...
            signal_savgol = savgol_filter(signal_a_filtrer, window_length=fenetre_opt, polyorder=1, mode=mode_filtre)

            # --- Comparaison des erreurs ---
            delta_fft = (x1_base - signal_fft) / np.max(x1_base)
            RMSE_fft = 100 * np.sqrt(np.mean(delta_fft ** 2))

            delta_savgol = (x1_base - signal_savgol) / np.max(x1_base)
            RMSE_savgol = 100 * np.sqrt(np.mean(delta_savgol ** 2))

            if RMSE_fft <= RMSE_savgol:
                 return signal_fft
            else:
        ...
        def extraire_harmoniques(signals, temps, nb_harmoniques):
            
            n = len(signals)
            dt = temps[1] - temps[0]
            freqs = np.fft.fftfreq(n, d=dt)
            fft_vals = np.fft.fft(signals)
        ...
        while True:
          ...
          #  plt.plot(timeC[:samples_per_period], ChampBdes, '-')
            ...
            ChampB, lag = Synchronisation(
                ChampBdes, ChampB[:samples_per_period],1)
           
            derivChampB, lag1 = Synchronisation(
                derivChampBdes, derivChampB[:samples_per_period],1)
      
            # filtrage du champ H
            ChampH2= extraire_harmoniques(ChampH2,timeC,1)
...
# === Calcul d'aire de cycle filtrer B(H) (interpolation) ===
    ...
    def start_filtrage(self): 
       ...
       #calcul Parametre filtrage harmonique 
