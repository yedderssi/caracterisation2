...
from analyse_fft_bh import FenetreAnalyse
...
class Worker(QThread):
    ...
    def run(self):
        ...
        def Synchronisation(ChampBdes, ChampB):
            ...
            Hilbert_ChampBdes = np.real(hilbert(ChampBdes))
            Hilbert_ChampB = np.real(hilbert(ChampB))
...
class MainWindow(QtWidgets.QMainWindow, Ui_JTcontrol):
    ...
    def ouvrir_fenetre_analyse(self):
         self.analyse_window = FenetreAnalyse()
         self.analyse_window.show()
         
    def ajouter_bouton_analyse(self):
         bouton = QPushButton("Analyse B(H)", self)
         bouton.clicked.connect(self.ouvrir_fenetre_analyse)
         self.layout().addWidget(bouton)  # adapte à ton layout
