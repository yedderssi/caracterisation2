...
import numpy as np
import csv
Hauteur = Di = De = Section = Rs = Rh = Freq = Ampli = Gain = Nbre_enregist = Mu = lm = Epaisseur = Nbre_Bandes = Largeur = alpha = beta = 0
Kf = 1
Nbre_periode = 2
iteration_max = 30
num_samples = 5000
...
class Worker(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.parent = parent
    def run(self):
        global Materiaux, Hauteur, Di, De, Section, Outils, Ns1, Ns2, Rs, Rh, Type, Forme, Freq, Ampli, Gain, Nbre_periode, Kf, Nm_ref, Mu, lm,num_samples,Resolution,Nbre_enregist,alpha,beta,iteration_max
        import ctypes
...
class ConfigWindow(QDialog, Ui_Dialog):
    ...
    def selectionchange(self, var):
        ...
        if (Outils == "Cadre Epstein personnalisé") or (Outils == "Cadre Epstein Standard"):
            ...
            self.lm_spin.setValue(0.94*1e3)
            self.De_spin.setDecimals(0)
            self.De_unite.setVisible(False)
        else:
            self.Hauteur_label.setText("Hauteur")
    ...
    def calcul_kf(self,var):
        ...
        if Di!=0 and De!=0 and Hauteur!=0:
            Stheo = Hauteur*1e3*(De*1e-3-Di*1e-3)/2
            Kf = var/Stheo
            self.Kf_spin.setValue(Kf)
        self.Kf_spin.valueChanged.connect(self.calcul_section)
    ...
    def calcul_section(self,var):
        ...
        self.section_spin.valueChanged.connect(self.calcul_kf)
    def accept(self):
        global Materiaux, Hauteur,Mu_air, Di, De, Section,alpha, beta, Outils, Ns1, Ns2, Rs, Rh, Type, Forme, Freq, Ampli, Gain, Nbre_periode, Kf, Nm_ref, Mu, lm,num_samples
        ...
        if lm == 0:
            ...
            lm*=1e3
        if Section==0:
            Section = De*Di*Hauteur
        main_window = self.parent()
        alpha = (Rs+Rh)*lm*1e-3/(Mu*Mu_air*Ns1)
...
class MainWindow(QtWidgets.QMainWindow, Ui_JTcontrol):
    ...
    def update_data(self, Outils, Materiaux, Di, De, Hauteur, Section, lm, Kf):
        if (Outils == "Cadre Epstein personnalisé") or (Outils == "Cadre Epstein Standard"):
            ...
            self.De_value.setText(str(int(De)))
        else:
        ...
        self.lm_value.setText(str(round(lm,3))+" mm")
    ...
    def opens_config(self):
        global Materiaux, Hauteur, Di, De, Section, Outils, Ns1, Ns2, Rs, Rh, Type, Forme, Freq, Ampli, Gain, Nbre_periode, Nbre_enregist, Nm_ref, Mu, Kf, num_samples,alpha,beta
