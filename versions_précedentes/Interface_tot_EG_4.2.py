...
Hauteur = Di = De = Section = Rs = Rh = Freq = Ampli = Gain = Nbre_enregist = mu_r = lm = Epaisseur = Nbre_Bandes = Largeur = alpha = beta = 0
Kf = 1
Nbre_periode = 2
iteration_max = 100
num_samples = 5000
...
class Worker(QThread):
    ...
    def run(self):
        ...
        """
        global Materiaux, Hauteur, Di, De, Section, Outils, Ns1, Ns2, Rs, Rh, Type, Forme, Freq, Ampli, Gain, Nbre_periode, Kf, Nm_ref, mu_r, lm, num_samples, Resolution, Nbre_enregist, alpha, beta, iteration_max
        global Fourier_test, func_eval, ChampBdes, Channel_ranges, Selection_Channel, timeC, bufferAMax, bufferAMin, bufferBMax, bufferBMin, bufferCMax, bufferCMin
        import ctypes
        ...
        from autograd import differential_operators
        ...
        def GBF(entree):
            ...
            ##
            global num_samples
            if (Freq*num_samples)/2>160000000:
                x=160000000/Freq-1
                print("limite sampling")
            else:
                x=len(entree)
            ...
            # i=len(entree)
            # while len(t)*Frequence>160000000:
            #     i-=1
        ...
        def PicoScope(Selection_Channel, Channelranges, entree):
            """
            Acquisition des données du Pico Scope.

            Parameters
            ----------
            Selection_Channel : tableau (3x4)
                Varaible qui permet de séléctionner la voie de l'oscillo. 
                ...
                EX pour la voie 1 : 
                        ...
                        u=1 : "setChA",
                        u=2 : "setdataBuffersA",
                        u=3 : 2 (Indice pour paramètrer la range du Pico),
            ...
            Returns
            -------
            BufferMax : Tableau (Nbre_enregistrement x Nbre de Mesures)
            ...
            for numero_channel in Selection_Channel:
                ...
                while True:
                    ...
                    chARange = ps.PS5000A_RANGE[Channel_ranges[numero_channel[3]]]
                    # analogue offset = 0 V
                    ...
                    if Num_resolution == 12 or Num_resolution == 16:
                        if (duration/num_samples) <= 8e-9:
                            timebase = round(
                                np.log(duration*500000000/num_samples)/np.log(2)+1)
                        else:
                ...
                Valeur_Max = np.max(BuffersMaxA[0])
                print(Valeur_Max)
            ...
            return BufferMax, cmaxSamples, timeIntervalns

        def Auto_Range(entree):
            """
            Fait une première acquisition des signaux pour obtenir leurs valeurs maximums et adapter la plage de mesure 

            Parameters
            ----------
            entree : Tableau
                Signale envoyé au GBF

            Returns
            -------
            Range : Tableau (1x3)
                Contient les indices à mettre dans le tableau "Channel_ranges" pour chaque voies 
            ...
            for numero_channel in Selection_Channel:
                ...
                while True:
                    ...
                    coupling_type = ps.PS5000A_COUPLING["PS5000A_DC"]
                    #chARange = ps.PS5000A_RANGE[Channel_ranges[current_range]]
                    chARange = ps.PS5000A_RANGE[Channel_ranges[5]]
                    # analogue offset = 0 V
                    status[numero_channel[1]] = ps.ps5000aSetChannel(
                        chandle, channel, 1, coupling_type, chARange, 0)
                    assert_pico_ok(status[numero_channel[1]])
                ...
                if Valeur_Max > 1000:
                    Numero_Range = 20e3
                    current_range = 9
                    while Valeur_Max < Numero_Range:

                        Numero_Range = Numero_Range*1e-3
                        Numero_Range //= 2
                    ...
                    if Valeur_Max < 1000 and Valeur_Max > 500:
                        current_range = 4
                    if Valeur_Max < 500 and Valeur_Max > 200:
                        current_range = 3
                    if Valeur_Max < 200 and Valeur_Max > 100:
                        current_range = 2
                    if Valeur_Max < 100 and Valeur_Max > 50:
                        current_range = 1
                    if Valeur_Max < 50 and Valeur_Max > 20:
                        current_range = 0
                    if Valeur_Max < 20:
                        current_range = -1
        ...
        def Mesure(Selection_Channel, Channel_ranges, entree):
            ...
            for numero_channel in Selection_Channel:
                ...
                for i in range(0, Nbre_enregistrement):
                    status["runBlock"] = ps.ps5000aRunBlock(
                        chandle, preTriggerSamples, postTriggerSamples, timebase, None, 0, None, None)
                    assert_pico_ok(status["runBlock"])
        ...
        def Mesure2(Selection_Channel, Channel_ranges, entree):
            """
            Acquisition des données du Pico Scope.
                ...
                while True:
                    ...
                    if Num_resolution == 12 or Num_resolution == 16:
                        if (duration/num_samples) <= 8e-9:
                         ...
                         timebase=1
                         maxSamples= int(Nbre_periode / (Frequence)*1e9/2**(timebase))
                        ...
                        if timebase<3:
                            timebase=3
                            maxSamples= int(Nbre_periode / (Frequence)*125000000/(timebase-2))

                            if maxSamples%2==0:
                                maxSamples= int(Nbre_periode / (Frequence)*125000000/(timebase-2))
                            else:
                                maxSamples= int(Nbre_periode / (Frequence)*125000000/(timebase-2))-1
                    ...
                    # if np.max(BuffersMax[0]) < np.max(adc2mV(BufferTest, ChRange, maxADC)):
                    #     break
        ...
        def Detection_Bornes(adc2mVChAMax, adc2mVChBMax, adc2mVChCMax):
            ...
            """
            V1 = adc2mVChAMax*Gain

            V1 = [adc2mVChAMax, np.negative(
                adc2mVChBMax), np.negative(adc2mVChBMax/Rs)]
        ...
        def Detection_Bornes2(adc2mVChAMax, adc2mVChBMax, adc2mVChCMax):
            ...
            V1 = [adc2mVChAMax, np.negative(adc2mVChBMax)]
            V1 = np.sum(V1, axis=0)  # Somme le long des colonnes
            ...
            fond_V1 = np.zeros(len(fft1))
            fond_V1[np.argmax(np.abs(fft1))] = np.max(np.abs(fft1))
            fond_V2 = np.zeros(len(fft2))
            fond_V2[np.argmax(np.abs(fft2))] = np.max(np.abs(fft2))

            v1_prim = ifft(fond_V1*np.exp(1j*np.angle(fft1))).real
            v2_prim = ifft(fond_V2*np.exp(1j*np.angle(fft2))).real

            V1_prim = fft(v1_prim)
            V2_prim = fft(v2_prim)

            correlation = np.corrcoef(
                np.angle(V1_prim), np.angle(V2_prim))[0, 1]
            if correlation >= 0:
                Bornes_homologues = "oui"
                return Bornes_homologues
            else:
        ...
        def Synchronisation(ChampBdes, ChampB, entree, samples_to_plotA):
            
        

            signal_correle = np.correlate(
                ChampBdes, ChampB, "same")
            lags = correlation_lags(
                ChampBdes.size, ChampB.size, mode="same")
            lag = lags[np.argmax(signal_correle)]

            # Transformée de Hilbert
            # Divisé par 50 pour sortie monitor (/100) ????
            signalA = np.roll(samples_to_plotA, lag)
            ...
            entree_aligne = np.abs(
                entree_analytic) * np.exp(1j * (np.angle(entree_analytic) + phase_diff))
            entree = np.real(entree_aligne)

            return entree, lag
        def start_from_zero(sig):
            zero_logical = (sig == 0) # Exact zero points
            zero_crossing_logical = (sig * np.roll(sig, 1) < 0) # Points where we cross zero without reaching it
            rising_crossing_logical = (zero_logical & np.roll(sig > 0, -1)) | (zero_crossing_logical & (sig > 0)) # Keep only the rising zeros from both previous set
            eligibles = np.where(rising_crossing_logical)[0] # Get the indexes
            choosen = np.argmin(np.min([eligibles, len(sig) - eligibles], axis=0)) # Make an only vector with the starting 0, then find the closest
            increment = -eligibles[choosen] + 1
            sig_out = np.roll(sig, increment)
            return sig_out, increment
        
        def Synchronisation2(ChampBdes, ChampB):
            """
            ...
            Pour un résultat plus précis on réaplique la transformée de Fourier puis on fait la différence de phase
            Le résultat est presque parfait, mais on peut ajouter une dernière boucle qui vas chercher le pssage au zéros pour avoir même point de départ 

            Cette fonction fonctionne avec des signeaux sinusoïdales complexes(avec du bruit) ou triangle
            ...
            # # Correlation des deux sinus obtenu et mesure su lag
            signal_correle = np.correlate(
            ...
            # Transformée de Fourier -> Pour un résultat plus précis

            # # On refait la transformée de Fourier du signal décalé
            # fft_ChampBdes = fft(ChampBdes)
            # fft_ChampB = fft(ChampB)
            ...
            # # or (x2[0]>0 and x2[1]>0) :
            # while (ChampB_filtre[0] < 0 and ChampB_filtre[1] < 0):
            #     ChampB_filtre = np.roll(ChampB_filtre,  -1)
            #     lag -= 1
            #     ChampB = np.roll(ChampB,  -1)
            
            
            
            ChampB= savgol_filter(ChampB, window_length=100, polyorder=3, mode="wrap")
        ...
        def func(entree,ChampB):
            global func_eval, ChampBdes
            
            if func_eval<5:
                ########
                # Envoie du signal au GBF
                entree2, pk = GBF(entree)

                # Acquisition des données
                BufferMax, cmaxSamples, timeIntervalns = Mesure2(
                    Selection_Channel, Channel_ranges, entree)

                # Nombre d'échantillon obtenue lors de l'aquisition
                ...
                Bornes_homologues = Detection_Bornes(
                    adc2mVChAMax, adc2mVChBMax, adc2mVChCMax)

                if Bornes_homologues == "non":
                ...
                # On ajuste l'entrée à notre échelle de temps
                # Uniquement pendant la première itération car entree a les bonnes dimensions ensuite
                if iteration == 0 or len(entree) != samples_per_period:
                ...
                V2_des = f4(timeC[:samples_per_period])
                ...
                ChampB, lag = Synchronisation2(
                    ChampBdes, ChampB[:samples_per_period])

                # ih = -samples_to_plotB/(Rsh*1000) # Multiplié par 1000 car mV ???
                ih = samples_to_plotB/(Rsh*1000)  # Multiplié par 1000 car mV ???

                ChampH2 = N1*ih/(lm*1e-3)
                ChampH2 = np.roll(ChampH2[:samples_per_period], lag)
                derivChampB = np.roll(derivChampB[:samples_per_period], lag)
                Tension_V2 = np.roll(Tension_V2[:samples_per_period], lag)

                # Filtre pour lisser le signal
                derivChampB = savgol_filter(
                    derivChampB, window_length=100, polyorder=3, mode="wrap")
                ########
            print(func_eval)
            # Vecteur de la sortie de la fonction à minimiser
            delta_B = np.abs((ChampBdes - ChampB))/np.max(ChampBdes)
            # delta_dB = (dB_dt_ref_sim - dB_dt_sim)/np.max(dB_dt_ref_sim)  #Vecteur de la sortie de la fonction à minimiser
            # Calcul de delta_V (incr�ment � ajouter au signal)
            erreur = np.sum(np.sqrt(delta_B**2))
            func_eval+=1
            #print(erreur)
            return erreur
        ...
        t = np.linspace(0, 1/int(Frequence), int(num_samples//Nbre_periode))
        #t = np.linspace(0, 1/int(Frequence),200)
        num_Fourier = int(num_samples/2+2)
        #num_Fourier = int(num_samples/4+1)
        ...
        if Forme == 'Sinusoïdale B':
            ...
            # with open('entree_triangulaire_01T.npy', 'rb') as f:
            #     entree = np.load(f)
            # entree=entree*1.25
        ...
        # entree = (alpha*ChampBdes+beta*derivChampBdes)#/Gain
        #entree = Amplitude*np.sin(w*t)
        f1 = interp1d(t, entree)
        # Mat_Broyden=0.5*np.max(fft(entree).real)*Mat_Broyden
        # plt.plot(derivChampBdes)
        
        # début de la boucle d'acquisition
        while True:
            # Envoie du signal au GBF
            entree1, pk = GBF(entree)
            ...
            adc2mVChBMax = np.mean(BufferMax[1], axis=0)
            adc2mVChCMax = np.mean(BufferMax[2], axis=0)
            #print("size dignal B 1 : ",adc2mVChBMax.size)
            
            #adc2mVChBMax=savgol_filter(adc2mVChBMax, window_length=200, polyorder=3, mode="wrap")
            ...
            #print("Taille TimeI 1 : ",timeI.size)
            #print(interval)
            #print("cmaxSamples : ",cmaxSamples.value)
            # On récupère la plage de temps pour ne garder qu'une période
            # Récupère le première indice supérieur à la période
            samples_per_period = np.where(timeI > (1/(Frequence)*1e9))[0][0]-1
            #print("samples par period 1 : ",samples_per_period)
            
                        
            adc2mVChAMa = interp1d(timeI, adc2mVChAMax, kind='slinear')
            adc2mVChBMa = interp1d(timeI, adc2mVChBMax, kind='slinear')
            adc2mVChCMa = interp1d(timeI, adc2mVChCMax, kind='slinear')
            x=num_samples

            while samples_per_period!= num_samples//2:
                x-=1
                timeI,interval = np.linspace(0, timeI[-1], x,retstep=True)
                samples_per_period = np.where(timeI > (1/(Frequence)*1e9))[0][0]-1


            samples_per_period = np.where(timeI > (1/(Frequence)*1e9))[0][0]-1
            #print("Taille TimeI 2 : ",timeI.size)
            ...
            #print("size dignal B 2 : ",adc2mVChBMax.size)
...
# Détection Bornes hanalogues
            ...
            #t1 = np.linspace(0, 1/int(Frequence), int(cmaxSamples//Nbre_periode))
            ...
            timeC = np.arange(np.array(samples_to_plotC).shape[0]) * (1/Frequence/samples_per_period)
            
            # print(timeC[:samples_per_period][-1])
            # print(t[-1])
            ...
            ChampB, lag = Synchronisation2(ChampBdes, ChampB[:samples_per_period])
            
            derivChampB, lag1 = Synchronisation2(derivChampBdes, derivChampB[:samples_per_period])
            ...
            # Filtre pour lisser le signal
            # derivChampB = savgol_filter(derivChampB, window_length=100, polyorder=3, mode="wrap")
            ...
            # Fourier_dB_ref, Fourier_dB, Fourier_B_ref, Fourier_B, Fourier_V = compute_fft_signals(
            #     ChampBdes, ChampB[:samples_per_period], entree, derivChampBdes, derivChampB[:samples_per_period])
            # Fourier_e_k = Fourier_V +(- alpha/5 * (Fourier_B_ref-Fourier_B) -beta/5 * (Fourier_dB_ref-Fourier_dB))
            # print('iter=', iteration, 'Amp_V=', round(np.max(entree), 3), 'delta_B=', round(np.max((Fourier_B_ref-Fourier_B)/np.max(Fourier_B_ref)), 3), 'delta_dB=', round(np.max((Fourier_dB_ref-Fourier_dB)/np.max(Fourier_dB_ref)), 3))
            ...
            coef = 2*Freq*num_samples/(10000*5000)
            ...
            # if THDN(derivChampB, num_samples) < 1 and cycle > 2:
            #     Mode_asservissement = 'PI_FFT'
            #     print("méthode PI_FFT")

            # else:
            #     Mode_asservissement = 'FFT_QN_dB_B'
            #     print("Méthode Quasi Newton")
            ...
            if Mode_asservissement == 'PI_FFT':
                ...
                rampe = np.concatenate(
                    (np.linspace(0.1, 0.5, num=5), 0.5*np.ones(iteration_max)))
                rampe *= reinit

                # if iteration <3:
                #     alpha=0.3
                #     beta=0.3
                # # elif THDN(derivChampB, num_samples) < THDN(derivChampBdes, num_samples)+1:
                # #     alpha= 0.2
                # #     beta=0.05
                # else :
                #     alpha= 0.5
                ...
                if Forme == 'Sinusoïdale B':
                    ...
                    # Algo V1
                    # if THDN(derivChampB, num_samples) < 4 and cycle > 2:
                    if RMSE<0.1 and cycle > 1:
                        alpha= 0.6*rampe[cycle]*coef
                        #alpha = 0.8*rampe[cycle]*coef
                        beta = 0.4*rampe[cycle]/coef
                        #beta = 0.4*rampe[cycle]/coef

                        print("allure réduite")

                    else:
                        alpha = 1.1*rampe[cycle]*coef
                        beta = 1.1*rampe[cycle]/coef
                        print("allure normale")

                if Forme == 'Triangulaire B':
                    print('réel=', THDN(derivChampB, num_samples),
                          ' désiré=', THDN(derivChampBdes, num_samples))
                    print('reel=', FF(derivChampB, timeC[samples_per_period]), ' |désiré=', FF(
                        derivChampBdes, timeC[samples_per_period]))
                    print(np.max(ChampB))
                    print("cycle", cycle)

                    # Algo V1
                    #if THDN(derivChampB, num_samples) < 40 and cycle > 5:
                    if RMSE<40 and cycle > 1:

                        alpha = 1*rampe[cycle]*coef
                        beta = 0.7*rampe[cycle]/coef
                        print("allure réduite2")

                    else:
                        alpha = 1*rampe[cycle]*coef
                        beta = 1*rampe[cycle]/coef
                        print("allure normale")

                Fourier_e_k = Fourier_V + alpha * (Fourier_B_des-Fourier_B_reel)/np.max(
                    Fourier_B_des) + beta * (Fourier_dB_des-Fourier_dB_reel)/np.max(Fourier_dB_des)
                #Fourier_e_k= Fourier_V  + theta * (Fourier_V2_des-Fourier_V2_reel)

                #Fourier_e_k= (Fourier_V  - alpha * (Fourier_B_des-Fourier_B_reel)/np.max(Fourier_B_des) - beta * (Fourier_dB_des-Fourier_dB_reel)/np.max(Fourier_dB_des))
                #print('iter=', iteration, 'Amp_V=', round(np.max(entree), 3), 'delta_B=', round(np.max((Fourier_B_ref-Fourier_B)/np.max(Fourier_B_ref)), 3), 'delta_dB=', round(np.max((Fourier_dB_ref-Fourier_dB)/np.max(Fourier_dB_ref)), 3))
                e_k = 0.5*len(entree)*np.fft.irfft(Fourier_e_k[0:int(
                    len(Fourier_e_k)/2)] - 1j * Fourier_e_k[int(len(Fourier_e_k)/2):])
                entree = e_k
                
                delta_B=np.max((Fourier_B_des-Fourier_B_reel) /np.max(Fourier_B_des))
                delta_dB=np.max((Fourier_dB_des-Fourier_dB_reel)/np.max(Fourier_dB_des))
                ...
                if Forme == 'Sinusoïdale B':
                    if (THDN(derivChampB, num_samples) > 8 or np.max(ChampB)>1.2*Ampli or np.max(derivChampB)>np.max(derivChampBdes)*1.2 or RMSE>np.min(RMSE_memoire))and cycle > 2:
                        print("init")
            ...
            elif Mode_asservissement == "PI_temp":
                alpha=0.5
                beta=0.0
                ...
                if Forme == 'Sinusoïdale B':
                    ...
                    # Algo V1
                    # if THDN(derivChampB, num_samples) < 4 and cycle > 2:
                    if RMSE<1 and cycle > 1:
                        alpha= 0.1*rampe[cycle]*coef
                        #alpha = 0.8*rampe[cycle]*coef
                        beta = 0.1*rampe[cycle]/coef
                        #beta = 0.4*rampe[cycle]/coef
                ...
                if Forme == 'Triangulaire B':
                    ...
                    if RMSE<4 and cycle > 1:
                        ...
                        alpha = 1.2*rampe[cycle]*coef
                        beta = 0.01*rampe[cycle]/coef
                        print("allure normale")
                ...
                entree=entree+alpha*(ChampBdes-ChampB)/np.max(ChampBdes)+beta*(derivChampBdes-derivChampB)/np.max(derivChampBdes)
                RMSE=math.sqrt(np.square(np.subtract(ChampB, ChampBdes)/np.max(ChampBdes)).mean())*100

                
            ###################################################################
            #entree=entree*10/Gain
            ...
            # print("tension GBF : ",np.max(entree))
            # print("tension V2 : ",np.max(adc2mVChCMax)/1e3)
            # print("tension Ushunt : ",np.max(adc2mVChBMax)/1e3)

            
            
            # if delta_B<delta_B_memoire and delta_dB<delta_dB_memoire:
            #     delta_B_memoire=delta_B
            #     delta_dB_memoire=delta_dB
            #     entree_memoire.append([entree,delta_B,delta_dB])

            # else:
            #     if iteration%2==0:
            #         print("mémoire")
            #         entree=entree_memoire+0.5*delta_B_memoire+ 0.5*delta_dB_memoire
            ...
            # RMSE = math.sqrt(np.square(np.subtract(
            #     derivChampB[:samples_per_period], derivChampBdes)/np.max(derivChampBdes)).mean())
            # print("RMSE : ",RMSE)
            ...
            if ((Amplitude < (np.max(ChampB)*1.05)) & (Amplitude > (np.max(ChampB)*0.95)) & (THDN(derivChampB, num_samples) < THDN(derivChampBdes, num_samples)+3) & ((FF(derivChampB, timeC[samples_per_period]) < FF(derivChampBdes, timeC[samples_per_period])*1.1) & (FF(derivChampB, timeC[samples_per_period]) > FF(derivChampBdes, timeC[samples_per_period])*0.9))) or iteration == iteration_max or self.parent.stop == True:
                self.parent.stop == False
                self.parent.stop_button.setEnabled(True)
                self.parent.iteration_label.setStyleSheet("color: red;")
                break
            iteration += 1
            print("timeC : ",timeC[samples_per_period])
        ...
        Aire = np.trapz(ChampB[samples_per_period//4:2*samples_per_period//4],x=ChampH2[samples_per_period//4:2*samples_per_period//4])      
        
        monteeB=np.concatenate((ChampB[3*samples_per_period//4:samples_per_period],ChampB[:samples_per_period//4]),axis=None)
        monteeH=np.concatenate((ChampH2[3*samples_per_period//4:samples_per_period],ChampH2[:samples_per_period//4]),axis=None)
        ...
        # with open('entree_triangulaire_01T.npy', 'wb') as f:
        #     np.save(f, entree)
...
class ConfigWindow(QDialog, Ui_Dialog):
    ...
    def accept(self):
        ...
        # if lm == 0:
        #     if Outils != "Cadre Epstein personnalisé" and Outils != "Cadre Epstein Standard":
        #         if De > 1.1*Di:
        #             lm = np.pi*((De*1e-3)-(Di*1e-3))/np.log((De*1e-3)/(Di*1e-3))
        #         else:
        #             lm = np.pi*((De*1e-3)+(Di*1e-3))/2
        #     elif Outils=="Cadre Epstein Standard":
        #         lm = 0.94
        #     lm*=1e-3
        if Section == 0:
    ...
    def saves_project(self):
        ...
        if selected_dir:
            ...
            csv_file = Nm_ref + "_" + str(Materiaux) + "_" + str(Forme) + "_" + str(
                self.Frequence) + "_" + str(round(self.Bmax, 2)) + ".csv"
