...
class Worker(QThread):
    ...
    def run(self):
        ...
        def Synchronisation(ChampBdes, ChampB):
            ...
            # CONV1 : la correction Hilbert ci-dessous reconstruisait un signal hybride
            # (amplitude enveloppe × phase de ChampBdes) ≠ ChampB mesuré.
            # Cela introduisait une distorsion à chaque itération et bloquait la convergence.
            # On utilise uniquement l'alignement par np.roll (suffisant pour la synchro de phase).
        ...
        def Asservissement(Mode_asservissement, ChampBdes, ChampB, derivChampBdes, derivChampB, entree, timeC, f_0, Mat_Broyden, reinit):
            ...
            if Mode_asservissement == 'PI FFT':
                ...
                # CONV3 : Back-Tracking atténué
                # - seuil RMSE : 15% de tolérance avant rollback (évite rollback sur bruit)
                # - reinit *= 0.7 au lieu de 0.3 (réduction moins agressive, évite blocage)
                # - cycle > 3 au lieu de > 2 (plus de marge avant 1er rollback)
                if Forme == 'Sinusoïdale B':
                    if (THDN(derivChampB, num_samples) > 8 or np.max(ChampB) > 1.5*Ampli or np.max(derivChampB) > np.max(derivChampBdes)*1.5 or RMSE > np.min(RMSE_memoire)*1.15) and cycle > 3:
                        ...
                        entree_memoire.append(entree)
                        cycle = 0
                        reinit = max(reinit * 0.7, 0.05)  # plancher 5% pour éviter gains nuls
                    else:
                        cycle += 1

                elif Forme == 'Triangulaire B':
                    if (THDN(derivChampB, num_samples) > 30 or np.max(ChampB) > 1.2*Ampli or np.max(derivChampB) > np.max(derivChampBdes)*1.2 or RMSE > np.min(RMSE_memoire)*1.15) and cycle > 3:
        ...
        def Connection():
            ...
            for port in serial_port: # on parcours tous les ports
                if port.device[:-1]=="/dev/ttyUMoyenneASB": # on récupère l'amplificateur branché sur le port USB
                    # VÉRIF3 : utilisation de 'with' pour garantir la fermeture du port série
        ...
        while True:
            ...
            # Calcul du champ B
            # CYCLE-FIX1 : detrend linéaire sur l'intégrale pour supprimer la dérive
            # (l'ancien correctif (Max+Min)/2 ne supprimait que l'offset DC, pas la dérive linéaire
            # due au résidu DC de V2, ce qui empêchait la fermeture du cycle B-H)
            integral = integrate.cumtrapz(
                (samples_to_plotC)/1000, timeC, initial=0)
            integral_corrige = detrend(integral, type='linear')   # supprime offset + dérive linéaire
           ...
           # for val in ChampB:
            ...
            # CONV2 : filtre(entree_tempo) supprimé — il effaçait les harmoniques
            # ajoutés par PI-FFT pour corriger la distorsion, bloquant la convergence.
            # Le signal entree envoyé au GBF est déjà lissé côté GBF (interpolation interne).
            # entree_tempo = filtre(entree_tempo)  # désactivé
            # entree_tempo = savgol_filter(entree_tempo, window_length=100, polyorder=3, mode="mirror")
            ...
            if RMSE < best_RMSE:
                best_RMSE = RMSE
                best_ChampB_tempo      = ChampB_tempo[:samples_per_period].copy()
                best_derivChampB_tempo = derivChampB_tempo[:samples_per_period].copy()
                best_ChampH2           = ChampH2_tempo[:samples_per_period].copy()
            ...
            if converge or iteration == iteration_max or self.parent.stop == True:
                ...
                if converge:
                    ...
                    # CYCLE-FIX2 : ChampH2_tempo est aligné sur ChampB_tempo (même phase)
                    ChampB      = ChampB_tempo
        ...
        finally:                 # VÉRIF2 : finished.emit() garanti même en cas d'exception
            self.finished.emit()
...
class ConfigWindow(QDialog, Ui_Dialog):
    ...
    def recalcul_geometrie(self):
        ...
        if Di > 0 and De > Di and Hauteur > 0:
            # Section effective du tore : S = (De - Di)/2 × Hauteur  [m²] → [mm²]
            S_mm2 = (De - Di) / 2.0 * Hauteur  # mm × mm = mm²
    ...
    def accept(self):
        ...
        if Section == 0 and Di > 0 and De > Di and Hauteur > 0:
            Section = round((De - Di) / 2.0 * Hauteur, 4)  # mm²
...
class DemagWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Signal de démagnetisation : cosinus 50 Hz à amplitude décroissante
            a, b = 0, 5
            t = np.linspace(a, b, num=1000)
            entree = np.exp(-t) * np.cos(2 * np.pi * t * 50)
            entree = entree[:int(len(entree) / 2)]
            entree = np.concatenate([entree, np.zeros(50)])
            t = t[:len(entree)]
