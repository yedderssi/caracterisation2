...
class Worker(QThread):
    ...
    def run(self):
        ...
        from scipy.signal import detrend, correlate, correlation_lags, blackmanharris, hilbert, savgol_filter, sawtooth, butter
        ...
        def symetrie(an, bn):
            """
            Permet suprimer rous les harmonique de rang paire car nous travaillons avec un sinus
            """
        ...
        def Asservissement(Mode_asservissement, ChampBdes, ChampB, derivChampBdes, derivChampB, entree, timeC, f_0, Mat_Broyden, reinit):
            ...
            if Mode_asservissement == 'PI FFT':
                ...
                # calcul de alpha et beta lors du back Tracking (si les mesure de RMSE augmente on diminu les gains pour converger plus rapidement)
                rampe_alpha *= reinit
                rampe_beta *= reinit
                ...
                e_k = 0.5*len(entree)*np.fft.irfft(Fourier_e_k[0:int(
                    len(Fourier_e_k)/2)] - 1j * Fourier_e_k[int(len(Fourier_e_k)/2):]) # trandformée inverse
                entree = e_k
        ...
        def filtre(signal_a_filtrer):
            ...
            min_RMSE = 51
            for num_harmoniques in range(1, 21):  # Tester de 1 à 20 harmoniques
        ...
        while True:
            ...
            V2=adc2mVChCMax # variable qui permet d'annalyser le bruit de la tension au secondaire
        ...
        Hc = np.mean(Hcs) # Calcule du champ coercitif
...
class ConfigWindow(QDialog, Ui_Dialog):
    ...
    def accept(self):
        ...
        lm = self.lm_spin.value()
        if Section == 0:
            Section = round(Hauteur*1e-3*((De*(1e-3))-(Di*(1e-3)))/2)
